 ...
 * @since 11.1
 */
public class AuditEventDomainProducer extends EventDomainProducer {

    protected static final String SOURCE_NAME = "audit";

    protected List<Record> records = new ArrayList<>();

    public AuditEventDomainProducer(String name, String stream) {
        super(name, stream);
    }
    ...
    protected Record buildRecordFromEvent(LogEntry entry) {
        ...
        if (entry.getLogDate() != null) {
            event.logDate = entry.getLogDate().getTime();
        } else {
            event.logDate = 0;
        }
        event.eventId = entry.getId();
        Codec<AuditEvent> codec = Framework.getService(CodecService.class).getCodec("avro", AuditEvent.class);
