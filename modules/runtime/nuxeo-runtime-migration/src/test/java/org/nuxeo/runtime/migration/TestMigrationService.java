...
public class TestMigrationService {
    ...
    public void testGetMigration() {
        assertDummyMigration(migrationService.getMigration("dummy-migration"), "dummy-migration", "Dummy Migration",
                "migration.dummy", "before", "Migrate dummy state from before to after");
    }
    ...
    public void testGetMigrations() {
        ...
        assertDummyMigration(migrations.get(0), "dummy-migration", "Dummy Migration",
                "migration.dummy", "before", "Migrate dummy state from before to after");
        assertDummyMigration(migrations.get(1), "dummy-multi-migration", "Dummy Multi Migration",
                "multi.migration.dummy", "before", "Migrate multi-dummy state from before to after");
    }
    ...
    protected void assertDummyMigration(Migration actual, String expectedId, String expectedDescription,
            String expectedDescriptionLabel, String expectedState, String expectedStepDescription) {
        assertEquals(expectedId, actual.getId());
        assertEquals(expectedDescription, actual.getDescription());
        assertEquals(expectedDescriptionLabel, actual.getDescriptionLabel());
        assertEquals(expectedState, actual.getStatus().getState());
        assertEquals(expectedStepDescription, actual.getSteps().get(0).getDescription());
    }
